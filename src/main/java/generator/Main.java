...
public class Main {
    ...
    private static <R extends Runnable> List<R> startThreads(
        final Supplier<R> runnableSupplier,
        int n
    ) {
    ...
    public static void main(String[] args)
        ...
        options.addOption(
            ...
            "updaterThreads",
            true,
            "number of treads to update documents"
        );
        options.addOption(
            "l",
            "minId",
            true,
            "min document id for updater threads"
        );
        options.addOption(
            "h",
            "maxId",
            true,
            "max document id for updater threads"
        );
        options.addOption(
            "i",
            "inserterThreads",
            true,
            "number of treads to insert new documents"
        );
        options.addOption(
            "s",
            "startId",
            true,
            "starting document id for inserter threads"
        );
        ...
        options.addOption(
            ...
            "targetOpen",
            true,
            "target # of dangling reads per thread"
        );
        options.addOption(
            "w",
            "cpuWasters",
            true,
            "Threads to spin and do nothing"
        );
        options.addOption(
            "rw",
            "cpuWastePerReadOp",
            true,
            "Time to waste in read, in s"
        );
        options.addOption(
            "ee",
            "errorEndpoint",
            true,
            "Endpoint to POST errors to"
        );
        ...
        if (!cmd.hasOption("uri")) {
            System.out.println(
                "must specify server URI, e.g. mongodb+srv://dmitry:<password>@cluster0.z1nie.mongodb-dev.net/?retryWrites=true&w=majority"
            );
            return;
        } else {
        ...
        if (cmd.hasOption("u")) {
            ...
            if ((!cmd.hasOption("l") || !cmd.hasOption("h"))) {
                System.out.println(
                    "must specify minId and maxId for updater threads"
                );
                return;
            }
        }
        final long minId = cmd.hasOption("l")
            ? Long.parseLong(cmd.getOptionValue("l"))
            : 0;
        final long maxId = cmd.hasOption("h")
            ? Long.parseLong(cmd.getOptionValue("h"))
            : 0;
        ...
        if (cmd.hasOption("n")) {
            ...
            settingsBuilder.transportSettings(
                TransportSettings.nettyBuilder().build()
            );
        } else {
        ...
        if (cmd.hasOption("f")) {
            ...
            Arrays.stream(cmd.getOptionValue("p").split(","))
            ...
            builder.addServerMonitorListener(
                new ServerMonitorListener() {
                    ...
                    public void serverHearbeatStarted(
                        final ServerHeartbeatStartedEvent event
                    ) {
                    ...
                    public void serverHeartbeatSucceeded(
                        final ServerHeartbeatSucceededEvent event
                    ) {
                        heartbeatLogger.info("Heartbeat succeeded {}", event);
                    }

                    @Override
                    public void serverHeartbeatFailed(
                        final ServerHeartbeatFailedEvent event
                    ) {
        ...
        try (MongoClient mongoClient = MongoClients.create(settings)) {
            ...
            List<Inserter> inserters = startThreads(
                () -> new Inserter(collection),
                numInserters
            );

            List<Updater> updaters = startThreads(
                () ->
                    new Updater(
                        updateCounter,
                        documentGenerator,
            ...
            List<Reader> readers = startThreads(
            ...
            final List<SocketChecker> socketCheckers = ports
                .stream()
                .map(port ->
                    startThreads(
                        () -> new SocketChecker(frontendId, port),
                        1
                    ).get(0)
                )
                .toList();
            ...
            while (true) {
                ...
                long insertCount = inserters
                    .stream()
                    .map(x -> x.myCount)
                    .reduce(0L, Long::sum);
                long updateCount = updaters
                    .stream()
                    .map(x -> x.myCount)
                    .reduce(0L, Long::sum);
                ...
                if (numInserters > 0) {
                    ...
                    throughputLogger.trace(
                        "avg writes/s " + insertCount / (timeDelta / 1e9)
                    );
                }
                //        if (numUpdaters > 0) {
    ...
    public static class Reader implements Runnable {
        ...
        public Reader(final MongoClientSettings settings) {
            ...
            collection = connection
                .getDatabase("bad_idea")
                .getCollection("against_docs_and_common_sense");
        }
                    ...
                    findResult.subscribe(
                        new Subscriber<Document>() {
                            ...
                            public void onComplete() {
                                ...
                                final var timeMs =
                                    (endTime - startTime) / 100_000;
                                if (timeMs > 200) {
                                    updaterLogger.info(
                                        "Read took a long time " +
                                        timeMs +
                                        " ms"
                                    );
                                } else {
                                ...
                                long spinEnd =
                                    ...
                                    (READ_SPIN * 1000L); // 1 minute from now
    ...
    public static class Updater implements Runnable {
        ...
        public void run() {
            while (flag) {
                ...
                try {
                    final var startTime = System.nanoTime();
                    var replacePublisher = collection.findOneAndUpdate(
                        query,
                        Updates.set("whatWasId", id + 1),
                    ...
                    if (duration >= 200) {
                        updaterLogger.warn(
                            "update took a long time " + duration
                        );
                    } else {
    ...
    public static class Inserter implements Runnable {
        ...
        private final Logger inserterLogger = LoggerFactory.getLogger(
            "timings"
        );
        private final DocumentGenerator _generator = new DocumentGenerator(
            null
        );
        public long myCount = 0;
        ...
        public void run() {
            ...
            while (flag) {
                ...
                try {
                    ...
                    final var endTime = System.nanoTime();
                    final var duration_millis =
                        (endTime - startTime) / 1_000_000;
                    if (duration_millis >= 200) {
                        inserterLogger.warn(
                            "insert took a long time " + duration_millis
                        );
                    } else {
                        inserterLogger.trace(
                            "insert took " + (endTime - startTime)
                        );
                    }
                    myCount += BATCH_SIZE;
                    //buffer.clear();
        ...
        public void run() {
            while (flag) {
                ...
                try (Socket socket = new Socket()) {
                    final var endpoint = new InetSocketAddress(
                        targetURI,
                        targetPort
                    );
